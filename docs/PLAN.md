...
| AI provider | Abstraction in `packages/ai`. Default adapter: **GitHub Copilot** (per-user OAuth device-flow → encrypted GH token at rest → requests sent to the officially-public GitHub Models inference API with the user's token as bearer). Fallbacks: OpenAI, Mock, Disabled. App must be fully functional with AI disabled. |
...
- `packages/ai`: stable `parseIntent(prompt, ctx) → ToolCall[]` interface with tool-schema source of truth (zod + JSON-schema for function calling). Providers: `DisabledProvider`, `MockProvider` (deterministic rules for dev/tests), `OpenAIProvider` (fetch-based, no SDK dep), and **`CopilotProvider`** (default — uses the OpenAI-compatible GitHub Copilot chat-completions endpoint).
- **GitHub Copilot / GitHub Models is the primary AI provider.** Each user connects their own GitHub account via OAuth **device flow** (client ID configurable, defaults to the public VS Code Copilot device-flow client). Requests are sent directly to the **officially public GitHub Models inference API** (`https://models.github.ai/inference/chat/completions`) using the user's GitHub token as the bearer — no undocumented endpoints, no session-token exchange. The long-lived GitHub access token is stored encrypted at rest in a new `github_accounts` table (AES-256-GCM, same envelope as Google refresh tokens). Models are addressed as `publisher/name` (default `openai/gpt-4o-mini`; any model on GitHub Models works — `meta/*`, `mistral-ai/*`, etc.). App remains fully functional with AI disabled or with users who haven't connected GitHub yet (returns `403 github_not_connected` instead of failing hard).
...
- Env: `HOME_OS_AI_PROVIDER` (default `copilot`; also `disabled`/`mock`/`openai`), `HOME_OS_GITHUB_CLIENT_ID` (default: VS Code Copilot device-flow client), `HOME_OS_COPILOT_MODEL` (default `openai/gpt-4o-mini`), `HOME_OS_COPILOT_BASE_URL` (default `https://models.github.ai`). OpenAI vars still present as a fallback provider.
...
- 22 tests in `packages/ai` (MockProvider, OpenAIProvider, **CopilotProvider** — auth via bearer GH token to `models.github.ai/inference`, custom model/baseUrl, error handling, invalid tool_call handling, createProvider matrix) + 10 AI API integration tests + 7 new GitHub device-flow integration tests (status, start, poll pending→ok with encrypted token persistence, disconnect).
...
| GitHub Models API/model availability changes | Provider abstraction swaps in OpenAI/Anthropic/Mock without touching routes; `HOME_OS_COPILOT_MODEL` is a single env var to pick a different publisher/model; app works without AI |
